...
static void intersectRaysWithBVH(const BVHInterface &bvh, const Scene &scene, std::span<Ray> rays);
...
struct Config {
    ...
    std::vector<CameraConfig> cameras {1};
};
        ...
        if (action == GLFW_PRESS) {
            switch (key) {
                case GLFW_KEY_R: {
                    ...
                    auto pixel = glm::ivec2(tmp * glm::vec2(config.windowSize));
                    glm::vec2 position = (glm::vec2(pixel) + 0.5f) / glm::vec2(config.windowSize) * 2.f - 1.f;
                    debugRays = { camera.generateRay(position) };
                } break;
                case GLFW_KEY_A: {
                    bvhDebugLeaf = std::min(bvhDebugLeaf + 1, (int) bvh.numLeaves() - 1);
                } break;
                case GLFW_KEY_S: {
                    bvhDebugLeaf = std::max(0, bvhDebugLeaf - 1);
                } break;
    ...
    while (!window.shouldClose()) {
        ...
        ImGui::Begin("Final Project");
        {
            ...
            if (ImGui::Combo("Scenes", reinterpret_cast<int*>(&sceneType), items.data(), int(items.size()))) {
                ...
                intersectRaysWithBVH(bvh, scene, debugRays);
            }
        }
        ...
        if (debugBVHLevel)
            ImGui::SliderInt("BVH Level", &bvhDebugLevel, 0, (int) bvh.numLevels() - 1);
        ImGui::Checkbox("Draw BVH Leaf", &debugBVHLeaf);
        if (debugBVHLeaf)
            ImGui::SliderInt("BVH Leaf", &bvhDebugLeaf, 0, (int) bvh.numLeaves() - 1);
...
void drawSceneOpenGL(const Scene& scene)
{
    ...
    // Tell OpenGL where the lights are (so it knows how to shade surfaces in the scene).
...
static void intersectRaysWithBVH(const BVHInterface &bvh, const Scene &scene, std::span<Ray> rays) {
    for (Ray &ray: rays) {
        ...
        HitData hitInfo;
        if (!bvh.intersect(scene, ray, hitInfo)) {
            drawRay(ray, glm::vec3(1.0f, 0.0f, 0.0f));
        }

        // Draw colored ray and the hit normal
        drawRay(ray, hitInfo.material.kd);
        drawRay(Ray{ray.origin + ray.direction * ray.t, hitInfo.normal, 0.1f}, glm::vec3(0.0f, 1.0f, 0.0f));
    }
}
