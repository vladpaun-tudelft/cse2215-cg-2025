...
InvisibleWindow::InvisibleWindow(std::string_view title, const glm::ivec2& windowSize, OpenGLVersion glVersion, bool presentable, bool visible)
...
{
    // Very uggly but works so whatever
    return;
    if (m_presentable) {
        switch (m_glVersion) {
        case OpenGLVersion::GL2: {
...
void InvisibleWindow::close()
{
    glfwSetWindowShouldClose(m_pWindow, 1);
}

bool InvisibleWindow::shouldClose()
{
    return glfwWindowShouldClose(m_pWindow) != 0;
}

void InvisibleWindow::updateInput()
{
    glfwPollEvents();
...
void InvisibleWindow::swapBuffers()
{
...
void InvisibleWindow::renderToImage (const std::filesystem::path& filePath, const bool flipY) {
...
void InvisibleWindow::registerKeyCallback(KeyCallback&& callback)
{
    m_keyCallbacks.push_back(std::move(callback));
}

void InvisibleWindow::registerCharCallback(CharCallback&& callback)
{
    m_charCallbacks.push_back(std::move(callback));
}

void InvisibleWindow::registerMouseButtonCallback(MouseButtonCallback&& callback)
{
    m_mouseButtonCallbacks.push_back(std::move(callback));
}

void InvisibleWindow::registerScrollCallback(ScrollCallback&& callback)
{
    m_scrollCallbacks.push_back(std::move(callback));
}

void InvisibleWindow::registerWindowResizeCallback(WindowResizeCallback&& callback)
{
    m_windowResizeCallbacks.push_back(std::move(callback));
}

void InvisibleWindow::registerMouseMoveCallback(MouseMoveCallback&& callback)
{
    m_mouseMoveCallbacks.push_back(std::move(callback));
}

void InvisibleWindow::keyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    ...
    const InvisibleWindow* pThisWindow = static_cast<const InvisibleWindow*>(glfwGetWindowUserPointer(window));
    for (auto& callback : pThisWindow->m_keyCallbacks)
        callback(key, scancode, action, mods);
}

void InvisibleWindow::charCallback(GLFWwindow* window, unsigned unicodeCodePoint)
{
...
void InvisibleWindow::mouseButtonCallback(GLFWwindow* window, int button, int action, int mods)
{
    // Ignore callbacks when the user is interacting with imgui.
...
void InvisibleWindow::mouseMoveCallback(GLFWwindow* window, double xpos, double ypos)
{
    // Ignore callbacks when the user is interacting with imgui.
    if (ImGui::GetIO().WantCaptureMouse)
        return;
...
void InvisibleWindow::scrollCallback(GLFWwindow* window, double xoffset, double yoffset)
{
    // Ignore callbacks when the user is interacting with imgui.
    if (ImGui::GetIO().WantCaptureMouse)
        return;
...
void InvisibleWindow::windowSizeCallback(GLFWwindow* window, int width, int height)
{
    InvisibleWindow* pThisWindow = static_cast<InvisibleWindow*>(glfwGetWindowUserPointer(window));
...
bool InvisibleWindow::isKeyPressed(int key) const
{
    return glfwGetKey(m_pWindow, key) == GLFW_PRESS;
}

bool InvisibleWindow::isMouseButtonPressed(int button) const
{
    return glfwGetMouseButton(m_pWindow, button) == GLFW_PRESS;
}

glm::vec2 InvisibleWindow::getCursorPos() const
{
    double x, y;
...
glm::vec2 InvisibleWindow::getNormalizedCursorPos() const
{
    return getCursorPos() / glm::vec2(m_windowSize);
}

glm::vec2 InvisibleWindow::getCursorPixel() const
{
...
void InvisibleWindow::setMouseCapture(bool capture)
{
    if (capture) {
...
glm::ivec2 InvisibleWindow::getWindowSize() const
{
    return m_windowSize;
}

glm::ivec2 InvisibleWindow::getFrameBufferSize() const
{
    glm::ivec2 out {};
...
float InvisibleWindow::getAspectRatio() const
{
...
float InvisibleWindow::getDpiScalingFactor() const
{
