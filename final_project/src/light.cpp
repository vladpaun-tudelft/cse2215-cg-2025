...
glm::vec3 computeContributionPointLight(RenderState& state, const PointLight& light, const Ray& ray, const HitInfo& hitInfo)
{
    ...
    bool visible = visibilityOfLightSampleBinary(state, light.position, light.color, ray, hitInfo);  // !! This is actually wrong implementation but not covered by unit tests
    return visible ? computeShading(state, v, l, light.color, hitInfo) : glm::vec3(0);
}
...
glm::vec3 computeContributionSegmentLight(RenderState& state, const SegmentLight& light, const Ray& ray, const HitInfo& hitInfo, uint32_t numSamples)
{
    ...
    glm::vec3 result(0);
    glm::vec3 v = -ray.direction;
    glm::vec3 p = ray.origin + ray.t * ray.direction + 0.001f * hitInfo.normal;
    for (auto i = 0; i < numSamples; ++i) {
        glm::vec3 lightPos, lightColor;
        sampleSegmentLight(state.sampler.next_1d(), light, lightPos, lightColor);
        glm::vec3 l = glm::normalize(lightPos - p);
        glm::vec3 Li = computeShading(state, v, l, lightColor, hitInfo);
        result += visibilityOfLightSample(state, lightPos, Li, ray, hitInfo);
    }
